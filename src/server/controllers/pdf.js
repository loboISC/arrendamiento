...
try {
  ...
  const logoPath = path.join(publicPath, 'assets/images/logo-demo.jpg');
  const isoPath = path.join(publicPath, 'assets/images/iso 9001.png');
...
exports.generarCotizacionPdf = async (req, res) => {
  ...
  try {
    ...
    // SOLUCIÓN PARA RECURSOS EN DOCKER/HTTPS:
    // Reemplazar la etiqueta <base> si existe para que apunte a la ruta local en el contenedor
    ...
    if (processedHtml.includes('<base href=')) {
      ...
      processedHtml = processedHtml.replace(/<base href="[^"]*">/i, `<base href="${localBaseUrl}">`);
    } else {
      // Insertar base href después del <head>
      processedHtml = processedHtml.replace(/<head>/i, `<head>\n    <base href="${localBaseUrl}">`);
    }

    // Cargar el contenido HTML procesado
