...
(function () {
  ...
  function getTokenFromURL() {
    try { return new URLSearchParams(location.search).get('token') || null; } catch (_) { return null; }
  }
  ...
  async function submitToServer() {
    ...
    const survey_slug = getSurveySlug();
    const answers = collectAnswers();
    const meta = { ua: navigator.userAgent, path: location.pathname, ts: Date.now() };
    const payload = { token, survey_slug, answers, meta };
