(function () {
    ...
    for (const r of rows) {
      ...
      if (summaryCols.img) {
        ...
        if (summaryCols.showImagen) {
          ...
          img.src = r.img || '/assets/images/logo-demo.jpg';
          img.alt = (r.clave || 'IMG');
  ...
  function renderTableRows(quotes) {
    ...
    for (const q of quotes) {
      const tr = document.createElement('tr');
      if (sel.clave || sel.imagen) { const td = document.createElement('td'); td.className = 'align-top'; const wrap = document.createElement('div'); wrap.className = 'flex items-center space-x-2'; if (sel.imagen) { const img = document.createElement('img'); img.src = q.imagen || '/assets/images/logo-demo.jpg'; img.alt = 'img'; img.className = 'w-10 h-10 object-cover rounded'; wrap.appendChild(img); } if (sel.clave) { const span = document.createElement('span'); span.className = 'font-semibold text-gray-800'; const claveFormatted = formatClaveWithPrefix(q.clave || q.folio || '-', q); span.textContent = claveFormatted; wrap.appendChild(span); } td.appendChild(wrap); tr.appendChild(td); }
