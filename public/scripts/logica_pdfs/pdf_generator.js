...
async function generatePDF(options = {}) {
  ...
  try {
    ...
    // CRÍTICO: Convertir textarea de condiciones a div con texto visible
    // Los textareas no preservan su valor al clonar, necesitamos convertirlo a texto visible
    try {
      const conditionsTextarea = clone.querySelector('#cr-summary-conditions');
      if (conditionsTextarea) {
        const conditionsValue = document.getElementById('cr-summary-conditions')?.value || '';
        const conditionsDiv = document.createElement('div');
        conditionsDiv.id = 'cr-summary-conditions';
        conditionsDiv.className = conditionsTextarea.className;
        conditionsDiv.style.cssText = conditionsTextarea.style.cssText;
        conditionsDiv.style.whiteSpace = 'pre-wrap';
        conditionsDiv.style.wordWrap = 'break-word';
        conditionsDiv.style.overflowWrap = 'break-word';
        conditionsDiv.style.resize = 'none';
        conditionsDiv.style.border = '1px solid #e5e7eb';
        conditionsDiv.style.borderRadius = '6px';
        conditionsDiv.style.padding = '8px';
        conditionsDiv.style.fontSize = '10.5px';
        conditionsDiv.style.lineHeight = '1.3';
        conditionsDiv.style.color = '#374151';
        conditionsDiv.style.backgroundColor = '#ffffff';
        conditionsDiv.style.height = 'auto';
        conditionsDiv.style.minHeight = '56px';
        conditionsDiv.textContent = conditionsValue || '';
        conditionsTextarea.parentNode.replaceChild(conditionsDiv, conditionsTextarea);
      }
    } catch (e) {
      console.warn('[PDF] Error al convertir textarea de condiciones:', e);
    }
    ...
    const logoDataUrl = await toDataURL((normalizedOrigin || '') + 'assets/images/logo-demo.jpg');
    const isoDataUrl = await toDataURL((normalizedOrigin || '') + 'assets/images/iso 9001.png');
