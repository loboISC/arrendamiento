...
(() => {
  ...
  // ============================================
  // Sincronizar información de envío desde el DOM al estado
  // ============================================
  function syncShippingInfoFromDOM(options = {}) {
    try {
      console.log('[syncShippingInfoFromDOM] Sincronizando datos de envío...');
      const rbBranch = document.getElementById('delivery-branch-radio');
      const isBranchDelivery = options.forceMethod !== undefined ? options.forceMethod : (rbBranch && rbBranch.checked);
      const shippingInfo = { method: isBranchDelivery ? 'branch' : 'home', timestamp: new Date().toISOString() };
      const contactCard = document.getElementById('cr-contact-card');
      shippingInfo.contact = {
        name: (contactCard?.querySelector('#cr-contact-name') || document.getElementById('cr-contact-name'))?.value?.trim() || '',
        phone: (contactCard?.querySelector('#cr-contact-phone') || document.getElementById('cr-contact-phone'))?.value?.trim() || '',
        email: (contactCard?.querySelector('#cr-contact-email') || document.getElementById('cr-contact-email'))?.value?.trim() || '',
        company: (contactCard?.querySelector('#cr-contact-company') || document.getElementById('cr-contact-company'))?.value?.trim() || '',
        mobile: (contactCard?.querySelector('#cr-contact-mobile') || document.getElementById('cr-contact-mobile'))?.value?.trim() || '',
        attn: (contactCard?.querySelector('#cr-contact-attn') || document.getElementById('cr-contact-attn'))?.value?.trim() || ''
      };
      if (isBranchDelivery) {
        shippingInfo.branch = state.selectedBranch || {
          id: document.getElementById('cr-branch-select')?.value || '',
          name: document.getElementById('cr-branch-name')?.textContent?.trim() || 'No seleccionada',
          address: document.getElementById('cr-branch-select')?.options[document.getElementById('cr-branch-select')?.selectedIndex]?.text?.split('—')[1]?.trim() || ''
        };
        shippingInfo.address = null;
        console.log('[syncShippingInfoFromDOM] Método: Sucursal', shippingInfo.branch);
      } else {
        shippingInfo.address = {
          ...
          country: document.getElementById('cr-delivery-country')?.value?.trim() || 'México',
          ...
          zone: document.getElementById('cr-zone-type')?.value || 'metropolitana'
        };
        shippingInfo.deliveryCost = parseFloat(document.getElementById('cr-delivery-cost')?.value || '0') || 0;
        shippingInfo.branch = null;
        console.log('[syncShippingInfoFromDOM] Método: Domicilio', shippingInfo.address);
      }
      state.shippingInfo = shippingInfo;
      try { if (window.state) window.state.shippingInfo = shippingInfo; } catch (e) { console.warn('[syncShippingInfoFromDOM] No se pudo actualizar window.state:', e); }
      console.log('[syncShippingInfoFromDOM] ✅ Datos sincronizados:', shippingInfo);
      return true;
    } catch (e) {
      console.error('[syncShippingInfoFromDOM] Error:', e);
      return false;
    }
  }
  window.syncShippingInfoFromDOM = syncShippingInfoFromDOM;
...
(() => {
  ...
  window.addEventListener('message', function (event) {
    try {
      // SI ESTAMOS EN MODO CLONACIÓN, NO INTERCEPTAR AQUÍ
      // El script cotizacion_venta_clone.js se encarga de procesar esta selección.
      if (sessionStorage.getItem('selecting-client-for-clone') === 'true') {
        return;
      }
  ...
  function collectQuotationData() {
    try {
      ...
      // ============================================
      // PARTE 1: ACCESORIOS SELECCIONADOS
      // ============================================
      console.log('[collectQuotationData] Recopilando accesorios...');
      let accesoriosSeleccionados = [];

      // Opción 1: Desde getAccessorySnapshot
      if (typeof getAccessorySnapshot === 'function') {
        try {
          const snapshot = getAccessorySnapshot();
          if (Array.isArray(snapshot) && snapshot.length > 0) {
            accesoriosSeleccionados = snapshot;
            console.log('[collectQuotationData] Accesorios desde snapshot:', accesoriosSeleccionados.length);
          }
        } catch (e) {
          console.warn('[collectQuotationData] Error obteniendo accesorios con getAccessorySnapshot:', e);
        }
      }

      // Opción 2: Desde state.accSelected si no hay snapshot
      if (accesoriosSeleccionados.length === 0 && state.accSelected && state.accSelected.size > 0) {
        accesoriosSeleccionados = Array.from(state.accSelected).map(accId => {
          const acc = state.accessories?.find(a => String(a.id) === String(accId));
          if (!acc) return null;

          return {
            id_accesorio: acc.id || accId,
            nombre: acc.nombre || acc.name || '',
            sku: acc.sku || acc.codigo || '',
            cantidad: 1,
            precio_unitario: parseFloat(acc.precio || acc.price || 0),
            subtotal: parseFloat(acc.precio || acc.price || 0)
          };
        }).filter(Boolean);
        console.log('[collectQuotationData] Accesorios desde state.accSelected:', accesoriosSeleccionados.length);
      }

      console.log('[collectQuotationData] Total accesorios:', accesoriosSeleccionados.length);

      // ============================================
      // PARTE 2: DATOS FINANCIEROS DEL DOM
      // ============================================
      console.log('[collectQuotationData] Obteniendo datos financieros del DOM...');

      const getFinancialValuesFromDOM = () => {
        try {
          // Subtotal
          ...
          const ivaText = ivaEl?.textContent?.trim() || '$0';
          const iva = parseFloat(ivaText.replace(/[$,]/g, '')) || 0;
          ...
          console.log('[getFinancialValuesFromDOM] Valores obtenidos:', { subtotal, iva, total });
          return { subtotal, iva, total };
        } catch (e) {
          console.warn('[getFinancialValuesFromDOM] Error, usando cálculo manual:', e);
      ...
      const { subtotal, iva, total } = getFinancialValuesFromDOM();
      // Calcular cantidad total de productos
      let cantidadTotal = 0;
      products.forEach(p => {
        cantidadTotal += parseInt(p.cantidad || 1);
      });
      console.log('[collectQuotationData] Cantidad total de piezas:', cantidadTotal);
      ...
      const getCurrentUserId = () => {
        try {
          console.log('[getCurrentUserId] Iniciando búsqueda de usuario...');

          // Opción 1: Desde localStorage con diferentes claves posibles
          const possibleKeys = ['usuario_logueado', 'currentUser', 'user', 'userData', 'session'];

          for (const key of possibleKeys) {
            try {
              const userStr = localStorage.getItem(key);
              if (userStr) {
                console.log(`[getCurrentUserId] Encontrado en localStorage.${key}:`, userStr.substring(0, 100));
                const user = JSON.parse(userStr);
                const id = user.id_usuario || user.id || user.userId || user.user_id || null;
                if (id) {
                  console.log(`[getCurrentUserId] ✅ ID encontrado en localStorage.${key}:`, id);
                  return id;
                }
              }
            } catch (e) {
              console.warn(`[getCurrentUserId] Error parseando ${key}:`, e);
            }
          }

          // Opción 2: Desde state
          if (state.currentUser?.id_usuario) {
            console.log('[getCurrentUserId] ✅ ID encontrado en state.currentUser:', state.currentUser.id_usuario);
            return state.currentUser.id_usuario;
          }

          // Opción 3: Desde window
          if (window.currentUser?.id_usuario) {
            console.log('[getCurrentUserId] ✅ ID encontrado en window.currentUser:', window.currentUser.id_usuario);
            return window.currentUser.id_usuario;
          }

          // Opción 4: Desde sessionStorage
          try {
            const sessionUser = sessionStorage.getItem('usuario_logueado') || sessionStorage.getItem('currentUser');
            if (sessionUser) {
              console.log('[getCurrentUserId] Encontrado en sessionStorage:', sessionUser.substring(0, 100));
              const user = JSON.parse(sessionUser);
              const id = user.id_usuario || user.id || user.userId || user.user_id || null;
              if (id) {
                console.log('[getCurrentUserId] ✅ ID encontrado en sessionStorage:', id);
                return id;
              }
            }
          } catch (e) {
            console.warn('[getCurrentUserId] Error en sessionStorage:', e);
          }

          console.warn('[getCurrentUserId] ❌ No se encontró ID de usuario en ninguna ubicación');
          console.log('[getCurrentUserId] localStorage keys:', Object.keys(localStorage));
          console.log('[getCurrentUserId] sessionStorage keys:', Object.keys(sessionStorage));
          return null;
        } catch (e) {
      ...
      const condiciones = document.getElementById('cr-summary-conditions')?.value || '';

      // Calcular total de accesorios (código existente...)
      const accesoriosTotal = accesoriosSeleccionados.reduce((sum, acc) => sum + (parseFloat(acc.subtotal) || 0), 0);
      if (accesoriosSeleccionados?.length) {
        console.log('[collectQuotationData] 🔧 Total accesorios:', accesoriosSeleccionados.length, 'Monto:', accesoriosTotal);
      }

      // ============================================
      // PARTE 3: DATOS DE ENVÍO COMPLETOS
      // ============================================
      console.log('[collectQuotationData] Procesando datos de envío...');

      // Construir dirección completa para el calendario
      const buildDireccionEntrega = () => {
        if (!state.shippingInfo) return null;

        if (state.shippingInfo.method === 'branch') {
          return `Recolección en Sucursal: ${state.shippingInfo.branch?.name || 'No especificada'}, ${state.shippingInfo.branch?.address || ''}`;
        } else {
          const addr = state.shippingInfo.address || {};
          const parts = [
            addr.street,
            addr.ext,
            addr.int ? `Int. ${addr.int}` : '',
            addr.colony,
            addr.city,
            addr.state,
            `C.P. ${addr.zip}`
      ...
      // Determinar tipo de envío correcto
      const getTipoEnvio = () => {
        if (!state.shippingInfo) return 'local';
      ...
      const getCostoEnvio = () => {
        if (state.shippingInfo?.deliveryCost) {
          return parseFloat(state.shippingInfo.deliveryCost) || 0;
        }

        const costoEl = document.getElementById('cr-delivery-cost');
        if (costoEl?.value) {
          return parseFloat(costoEl.value) || 0;
        }
      ...
      const getTipoZona = () => {
        const zona = state.shippingInfo?.address?.zone ||
          document.getElementById('cr-zone-type')?.value ||
          'metropolitana';
        // Capitalizar primera letra para cumplir con constraint de BD
        return zona.charAt(0).toUpperCase() + zona.slice(1).toLowerCase();
      };
      const tipoZona = getTipoZona();

      const direccionEntrega = buildDireccionEntrega();
